...
# Test robusto y determinista: lanza coordinador y workers en primer plano, espera a que todos terminen y verifica los logs
...
def test_worker_failure():
    limpiar_fs()
    crear_archivos_pg()
    subprocess.run("go run cmd/seq/mainseq.go plugins/wc.so filesystem/pg/pg-*.txt", shell=True, check=True)
    ...
    print("[TEST WORKER FAILURE] OK ☺️")
